...
system.DAE.y[system.Bus.v[29]] = round(system.DAE.y[system.Bus.v[29]], 14)
vv = [system.DAE.y[system.Bus.v[29]]]
tt = [0.0]
pyiter = []

# 输电线路故障模拟
system.StoFault.setup()  # 建立线路断开时间和线路恢复连接时间列表
[MWs, MWr] = system.Line.flow()
linep = copy.deepcopy(abs(MWs.real()))
system.StoFault.settimes(linep, t)
fixed_times = system.StoFault.gettimes(fixed_times)
fixed_times = sorted(fixed_times)
...
while t < system.Settings.tf and t+h > t and not diff_max:
    ...
    if switch:
        ...
        system.StoFault.intervention(actual_time)
    ...
    # yu 随机负荷波动
    # system.PQ.suiji(mup, muq, sigmap, sigmaq)
    ...
    k = k+1
    vv15 = round(system.DAE.y[system.Bus.v[29]].real, 14)
