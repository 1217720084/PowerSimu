...
def parsedata():
    ...
    for each_line in case:
        ...
        if data[0] == 'Avr2':
            ...
            Ae = float(data[12])
            Be = float(data[13])
            Avr2_case = {'bus': bus, 'Type': Type, 'vrmax': vrmax, 'vrmin': vrmin, 'Ka': Ka, 'Ta': Ta, 'Kf': Kf,
                         'Tf': Tf,
        ...
        if data[0] == 'Avr1':
            ...
            Ae = float(data[12])
            Be = float(data[13])
            Avr1_case = {'bus': bus, 'Type': Type, 'vrmax': vrmax, 'vrmin': vrmin, 'K0': K0, 'T1': T1, 'T2': T2,
                         'T3': T3,
...
def pf20180410(pi):
    ...
    convergence = True  # 收敛
    system.Settings.error = 1e-6
    tol = copy.deepcopy(system.Settings.error)
    cycle = True
    err = []
