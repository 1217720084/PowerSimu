...
system.DAE.y[system.Bus.v[15]] = round(system.DAE.y[system.Bus.v[15]], 6)
vv = [system.DAE.y[system.Bus.v[15]]]
tt = [0.0]
# 主循环
while t < system.Settings.tf and t+h > t and not diff_max:
    ...
    while system.Settings.error > tol and iteration < iter_max:
        ...
        if system.Settings.method == 1:  # 采用欧拉法
            ...
            if k == 1:  # CyMrH: 第一次用隐式梯形法启动，之后用Adams 2步法
            ...
            # CyMrH: Adams 2步法 Ac矩阵???
            else:
                system.DAE.Ac = sparse(
                    [[identica - 5 * h / 12 * system.DAE.Fx, system.DAE.Gx], [-5 * h / 12 * system.DAE.Fy, system.DAE.Gy]])

                system.DAE.tn = system.DAE.x - xa - h / 12 * (5 * system.DAE.f + 8 * fn - fn_1)
        # 限幅器
    ...
    k = k + 1
    vv15 = round(system.DAE.y[system.Bus.v[15]].real, 6)
