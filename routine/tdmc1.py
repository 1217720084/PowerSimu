...
if __name__ == '__main__':
    ...
    for i in range(n):
        ...
        # 负荷波动参数
        for pi in range(system.PQ.n):
            a = random.normalvariate(mup[pi], sigmap)
            b = random.normalvariate(muq[pi], sigmaq)
            system.PQ.Pl[pi] = system.PQ.Pl[pi]
            system.PQ.Ql[pi] = system.PQ.Ql[pi]

        # 风速波动

        for wi in range(system.Wind.n):
            wn = len(system.Wind.time[wi])
            for j in range(wn-1):
                c = random.normalvariate(0, sigmavw)
                system.Wind.svw[wi][j+1] = system.Wind.svw[wi][j+1] + c
    ...
    for i in range(n):
        datanew = 'C:\\Users\\user\Desktop\powercon2018\仿真结果\\14bus\案例2(全过程和只考虑初始的对比)\\conventional\\2\\' + 'tdvar' + str(
            i + 1) + '.mat'
