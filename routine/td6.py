...
def td(htype, syst, Pl, Ql, sn, ssvw, tf, tc, Sline):
    # 输出采用的时域仿真算法
    tstart = time.time()
    ...
    fixed_times.extend(system.Breaker.gettimes())
    # fixed_times = system.Fault.gettimes().extend(system.Breaker.gettimes())
    ...
    while t < system.Settings.tf and t + h > t and not diff_max:
        ...
        # 随机负荷波动
        # t1 = time.time()
        system.PQ.Pl = list(Pl)
        system.PQ.Ql = list(Ql)
        # t2 = time.time()
        # print('多进程通信耗时1：%s' % (t2 - t1))

        # 风速波动
        system.Wind.svw = list(ssvw)
        # system.DAE.x[system.Wind.vw] = vw
        # system.Wind.vwt = list(vw)
    ...
    # var = matrix(var)
    # var = var.T
    # sio.savemat('C://Users//user//Desktop//平台//余伟洲//仿真结果//RES 时域仿真//restdvar.mat', {'var': var, 't': t0, 'svw': system.Wind.svw})
...
if __name__ == '__main__':
    ...
    pf()
    # 备份变量
    tf0 = copy.deepcopy(system.Line.tf)
    tc0 = copy.deepcopy(system.Line.tc)
    lineu0 = copy.deepcopy(system.Line.u)
    xa = copy.deepcopy(system.DAE.x)
    ya = copy.deepcopy(system.DAE.y)
    aPl = copy.deepcopy(system.PQ.Pl)
    aQl = copy.deepcopy(system.PQ.Ql)
    svw0 = copy.deepcopy(system.Wind.svw)

    # 设置初始仿真时间
    system.DAE.t = system.Settings.t0
    # system.Settings.tf = 60  # 设置仿真终止时间
    # t = system.Settings.t0
    ...
    for i in range(n):
        ...
        td(htype='fixed', syst=syst, Pl=Pl, Ql=Ql, sn=sn, ssvw=ssvw, tf=tf, tc=tc, Sline=Sline)

        # 断路器复原
        if system.Breaker.u[0] == 0:
            system.Breaker.u[0] = 1
        else:
            system.Breaker.u[0] = 0
        system.Line.setstatus(system.Breaker.line[0], system.Breaker.u[0])
        t0 = matrix(system.Varout.t)
        var = system.Varout.var
        ...
        system.Line.u = copy.deepcopy(lineu0)
        system.Wind.svw = copy.deepcopy(svw0)

        print('第%i 次仿真' % (i + 1))
        datanew = 'C:\\Users\\user\Desktop\powercon2018\仿真结果\\14bus\案例2(全过程和只考虑初始的对比)\\agent\\nopss新1\\' + 'tdvar' + str(
            i + 1) + '.mat'

        sio.savemat(datanew, {'var': names['var%s' % (i + 1)], 't': names['time%s' % (i + 1)]})

    # for i in range(n):
    #     datanew = 'C:\\Users\\user\Desktop\powercon2018\仿真结果\\14bus\案例2(全过程和只考虑初始的对比)\\agent\\3\\' + 'tdvar' + str(i + 21) + '.mat'
    #
    #     sio.savemat(datanew, {'var': names['var%s' % (i + 1)], 't': names['time%s' % (i + 1)]})
    # p1.join()
    # p2.join()
